...
obs=DWR.load_observations('prmsl',
                          ...
                          dte+datetime.timedelta(hours=12))
# Run the quality control checks
obs['plausible']   = DIYA.qc_plausible_range(obs)
obs['first_guess'] = DIYA.qc_first_guess(obs,version='2c')
...
for station in stations:
    try:
        interpolated[station]=DWR.at_station_and_time(
                                          obs,station,dte)
        quality[station]=DIYA.qc_at_station_and_time(
                                          obs,station,dte)
    except StandardError:
        interpolated[station]=None
        quality[station]=None
...
# Output as png
fig.savefig('Scatter+contour+qc_%04d%02d%02d%02d.png' % 
                                    (year,month,day,hour))
