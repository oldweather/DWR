...
start_day=datetime.datetime(1953, 2, 1, 13)
end_day  =datetime.datetime(1953, 2, 28, 11)

# Function to check if the job is already done for this timepoint
def is_done(year,month,day,hour):
    op_file_name=("%s/images/DWR/vcs_20CR_comparison_1953/"+
                  "comparison_%04d%02d%02d%02d%02d.png") % (
...
while current_day<=end_day:
    ...
    while max_new_jobs>0 and current_day<=end_day:
        ...
        f.write('#!/bin/ksh -l\n')
        f.write(("#SBATCH --output=%s/sc_comparison_frame_"+
                 "%04d%02d%02d%02d.out\n") %
                   (opdir,
        ...
        for fraction in (0,.25,.5,.75):
            if is_done(current_day.year,current_day.month,
                ...
                continue
            cmd=("./comparison.py --year=%d --month=%d "+
                 "--day=%d --hour=%f &\n") % (
                   current_day.year,current_day.month,
                   current_day.day,current_day.hour+fraction)
            f.write(cmd)
