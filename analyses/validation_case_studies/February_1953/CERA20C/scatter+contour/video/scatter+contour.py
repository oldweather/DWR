...
parser.add_argument("--opdir", help="Directory for output files",
                    default=("%s/images/DWR/vcs_cera20c_scatter+contour" % 
                                             os.getenv('SCRATCH')),
...
obs_month=DWR.load_observations('prmsl',
                                datetime.datetime(1953,2,1,1),
                                datetime.datetime(1953,2,28,23))
# Remove stations already in ISPD
obs_month=obs_month[~obs_month['name'].isin(['LERWICK','STORNOWAY',
                                             'VALENTIA','CULDROSE',
                                             'GORLESTON','LEUCHARS'])]
...
wm.plot_obs(ax_map,obs_month,lat_label='latitude',
            lon_label='longitude',radius=0.25,facecolor=(0,0,0,0))
...
for y in range(0,len(stations)):
    ...
    ax_scp.scatter(ensemble.data/100.0,
